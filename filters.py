...
def create_filters(
    ...
    if distance_min is not None:
        dmin = DistanceFilter(operator.ge, distance_min)
        filters.append(dmin)
    if distance_max is not None:
        dmax = DistanceFilter(operator.le, distance_max)
        filters.append(dmax)

    if velocity_min is not None:
        vmin = VelocityFilter(operator.ge, velocity_min)
        filters.append(vmin)
    if velocity_max is not None:
        vmax = VelocityFilter(operator.le, velocity_max)
        filters.append(vmax)

    if diameter_min is not None:
        diamin = DiameterFilter(operator.ge, diameter_min)
        filters.append(diamin)
    if diameter_max is not None:
        diamax = DiameterFilter(operator.le, diameter_max)
        filters.append(diamax)
    ...
    if date is not None:
        dateeq = DateFilter(operator.eq, date)
        filters.append(dateeq)   

    if start_date is not None:
        datemin = DateFilter(operator.ge, start_date)
        filters.append(datemin)   
    if end_date is not None:
        datemax = DateFilter(operator.le, end_date)
        filters.append(datemax)
